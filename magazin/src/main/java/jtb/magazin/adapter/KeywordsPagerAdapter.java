...
import jtb.magazin.UI.SearchFrame;
import jtb.magazin.store.KeywordsArticleStore;
//podobna trieda ako JetPagerAdapter
public class KeywordsPagerAdapter extends PagerAdapter {
	
	DisplayImageOptions options;
	private Context mContext;
	ImageLoader imageLoader;
	KeywordsArticleStore store;
	int rowHeight;
	int rowWidth;
	int rowLastWidth;
	int rowLastHeight;
	...
	public KeywordsPagerAdapter(Context context, String keywordID) {
		
		mContext = context;
		this.options=((MainActivity)context).options;
		this.imageLoader=((MainActivity)context).imageLoader;
		
		//vytvori sa store s datami pre dane keyword, zvysok je rovnaky ako pri JetPagerAdapter s tym rozdielom ze jedna strana gridu
		//obsahuje 20 clankov
		store = new KeywordsArticleStore(keywordID,context);
	  	
		float mf= ((MainActivity)context).screenWidth;
		m=(int) (mf/Constants.MARGIN_WIDTH_KOEF);
    	float width = mf-6*m;
    	float height = ((MainActivity)context).screenHeight-5*m; 
	...
	public Object instantiateItem(ViewGroup view, int pos) {
		...
		int column = 5;
         ...
         g.setRowCount(row);    
             ...
             int k =0;
             if (pos==0) {
            	 //prve okienko musi byt staticke s poctom clasnkov
            	 FrameLayout f1 = new SearchFrame(mContext,store.getCount());
             ...
             for(int i=k;i<20;i++){  
            	 ...
            	 f1 = new ArticleThumbnailFrame(mContext, store.getItem(pos*20+i-1),params.get(i).height);
